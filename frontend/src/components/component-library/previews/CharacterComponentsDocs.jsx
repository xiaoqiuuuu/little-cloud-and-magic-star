...
function ExampleContent() {
  return <div className="cl-character-content"><span>CHARACTER SLOT</span><h3>人物装饰内容区</h3><Button size="small">操作按钮</Button></div>;
}
  ...
  return (
    <article className="cl-docs">
      <header className="cl-docs-header">
        <div><div className="cl-kicker">CHARACTER · BRAND</div><h1>人物组件</h1></div>
        ...
        <div className="cl-character-theme-groups">
          {Object.values(themePresets).map((preset) => (
            <div className="cl-character-theme-group" key={preset.id}>
              <header><strong>{preset.name}</strong></header>
                ...
                {preset.characterPackIds.map((packId) => {
                  ...
                  return (
                    <button type="button" key={pack.id} className={characterPackId === pack.id ? 'is-active' : ''} onClick={() => selectCharacterPack(pack.id)}>
                      <img src={pack.assets.buttonAvatar} alt="" /><span><strong>{pack.name}</strong></span>
                    </button>
                  );
                })}
              </div>
      ...
      <section className="cl-section">
        <div className="cl-section-heading"><div><span className="cl-section-index">03</span><h2>CharacterCard</h2></div></div>
      ...
      <section className="cl-section">
        ...
        <div className="cl-character-empty-grid">
          ...
          <CharacterEmptyState character={characterPackId === 'chengdu' ? 'xiaoyun' : 'chengdu'} title="没有搜索结果" action={<Button size="small" variant="secondary">清除筛选</Button>} />
        </div>
      </section>

      <section className="cl-section">
        <div className="cl-section-heading"><div><span className="cl-section-index">05</span><h2>使用方式</h2></div></div>
        <div className="cl-code-block"><pre><code>{`<CharacterButton>操作按钮</CharacterButton>\n\n<CharacterCard layout="corner">\n  <YourContent />\n</CharacterCard>\n\n<CharacterEmptyState\n  title="暂无数据"\n  action={<Button>新建</Button>}\n/>`}</code></pre></div>
      </section>
    </article>
  );
}
